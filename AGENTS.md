...
## Linter / LSP diagnostics — глобальное игнорирование

Если в Neovim линтер (pyright, mypy, ruff, любой LSP) выдаёт шумную
диагностику, которую нужно скрыть **везде и всегда** (а не через
`# type: ignore` в конкретном месте), — добавь подстроку сообщения в список
`ignored_diagnostic_patterns` в `home/nixvim/lsp.nix`.

Пример существующего паттерна:
- `"union syntax cannot be used with string operand"` — pyright шумит на
  `X | "Y"` (mixed forward-reference); чинить каждый файл нецелесообразно.

Правила:
1. Паттерн — **подстрока в lower-case** от `d.message`, сравнение через
   `msg:find(pat, 1, true)` (plain, без regex).
2. Пиши коротко и однозначно, чтобы не заглушить лишнее.
3. Рядом в комментарии `-- ...` кратко объясняй **почему** игнорируем.
4. Не глушить целые категории (severity/rule) — фильтруем **по тексту**,
   чтобы не потерять полезные диагностики от того же правила.
5. Если проблема специфична для одного проекта — предпочитай
   `pyproject.toml` / `pyrightconfig.json` в самом проекте, а не глобальный
   фильтр в nvim-конфиге.
